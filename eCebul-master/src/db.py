...
@connection
async def list_item(connect, shop_id: int):
    async with connect.execute("""
        SELECT item_name FROM ITEM WHERE shop_id = ? ORDER BY item_id DESC
    """, (shop_id,)) as cursor:
        if (res := (await cursor.fetchall())):
            return res
...
@connection
async def fund_shop(connect, id: int):
    async with connect.execute("""
        SELECT shop_fund FROM SHOP WHERE shop_id = ? 
    """, (id,)) as cursor:
        if (res := (await cursor.fetchall())):
            return res
            
@connection
async def create_shop(connect, owner_id: int, title: str, fund_id: int, default_message: str):
    owner_id, fund_id = int(owner_id), int(fund_id)
    await connect.execute("""
        INSERT INTO SHOP (shop_title, shop_owner_id, shop_fund, default_message) VALUES (?, ?, ?, ?)
    """, (title, owner_id, fund_id, default_message))
    
@connection
async def create_item(connect, count: int, name: str, desc: str, s_id: int, cost: int, status: str):
    await connect.execute("""
        INSERT INTO ITEM (item_count, item_name, description, shop_id, cost, status) VALUES (?, ?, ?, ?, ?, ?)
    """, (count, name, desc, s_id, cost, status))
    

@connection
async def view_item_id(connect, shop: int, name: str):
    async with connect.execute("""
            SELECT item_id FROM ITEM WHERE shop_id = ? AND item_name = ?
        """, (shop, name)) as cursor:
            if (res := (await cursor.fetchone())):
                return res
                
@connection
async def current_item(connect, id: int):
    async with connect.execute("""
            SELECT item_count, item_name, description, shop_id, cost, status FROM ITEM WHERE item_id = ? 
        """, (id,)) as cursor:
            if (res := (await cursor.fetchall())):
                return res
@connection
async def my_item(connect, id: int):
    async with connect.execute("""
            SELECT item_id, item_name, description, status FROM BOUGHT_ITEM WHERE owner_item_id = ? 
        """, (id,)) as cursor:
            if (res := (await cursor.fetchall())):
                return res
                
@connection
async def buy_item(connect, user_id: int, item_id: int, name: str, description: str, status: str, count: int):
    await connect.execute("""
        INSERT INTO BOUGHT_ITEM (owner_item_id, item_id, item_name, description, status) VALUES (?, ?, ?, ?, ?)
    """, (user_id, item_id, name, description, status))
    await connect.execute("""
        UPDATE ITEM SET item_count = ? WHERE item_id = ?
    """, (item_id, count))
